...
const MapForCard: React.FC = ({club}) => {
...
//   const userLongitude = useAppSelector((state) => state.search.longitude);
  const chosenClubLatitude = useAppSelector((state) => state.club.chosenClubLatitude);
  const chosenClubLongitude = useAppSelector((state) => state.club.chosenClubLongitude);
 ...
 useEffect(() => {
    const initialMap = new Map({
      ...
      view: new View({
        center: fromLonLat([chosenClubLongitude || initialCoordinates[0], chosenClubLatitude || initialCoordinates[1]]),
        zoom: 12,
      }),
  ...
  useEffect(() => {
    if (map && overlay) {
      ...
      const marker = new Feature({
        geometry: new Point(fromLonLat([chosenClubLongitude, chosenClubLatitude])),
      });
